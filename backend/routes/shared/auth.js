...
// GET /api/auth/users/:username - public read-only profile for any user
router.get("/users/:username", async (req, res) => {
  try {
    ...
    const user = await db.collection("users").findOne(
      { username: username, role: { $ne: "admin" } },
    ...
    // Count how many trips this user has organized vs joined
    const tripsOrganized = await db.collection("trips").countDocuments({
      organizer: new ObjectId(user._id),
    });

    const completedOrganized = await db.collection("trips").countDocuments({
      organizer: new ObjectId(user._id),
      status: "completed",
    });
    ...
    res.json({
      ...
      tripsOrganized,
      tripsAttended: user.tripsAttended || 0,
      completedOrganized,
