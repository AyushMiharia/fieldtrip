...
router.get("/", async (req, res) => {
  try {
    ...
    // Enrich each trip with organizerUsername if missing (backwards compat with seeded data)
    const tripsNeedingLookup = trips.filter((t) => !t.organizerUsername && t.organizer);
    if (tripsNeedingLookup.length > 0) {
      const organizerIds = [...new Set(tripsNeedingLookup.map((t) => t.organizer.toString()))].map(
        (id) => new ObjectId(id)
      );
      const users = await db
        .collection("users")
        .find({ _id: { $in: organizerIds } }, { projection: { username: 1 } })
        .toArray();
      const usernameMap = Object.fromEntries(users.map((u) => [u._id.toString(), u.username]));
      trips.forEach((t) => {
        if (!t.organizerUsername && t.organizer) {
          t.organizerUsername = usernameMap[t.organizer.toString()] || null;
        }
      });
    }
