...
function TripCard({ trip, user, onJoin, onLeave, onComplete, onCancel, onReview, showActivity }) {
  ...
  return (
    <article className="trip-card" aria-label={`Trip: ${trip.activityTitle || "Trip"} on ${new Date(trip.dateTime).toLocaleDateString()}`}>
      ...
      <div className="trip-card-body">
        <div className="trip-card-info">
          ...
          <span>
            👤 Organized by{" "}
            {trip.organizerUsername ? (
              <Link to={`/users/${trip.organizerUsername}`} className="organizer-link">
...
TripCard.propTypes = {
  trip: PropTypes.shape({
    ...
    organizerUsername: PropTypes.string,
